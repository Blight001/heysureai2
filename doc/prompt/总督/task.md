...
- 通过 `message.send+to+ai` 跟进进度、接收汇报，监督执行与异常。
- 汇总最终结果给用户（`message.send+to+user`），先结论+风险+下一步，再附证据。
...
- 通过 message.send+to+ai 向成员发送 inquiry 或 notify（带必要上下文）
...
- 最终向用户汇报时，优先使用 message.send+to+user（当需要图片/文件时使用 media_* 参数发送截图或关键证据）。
