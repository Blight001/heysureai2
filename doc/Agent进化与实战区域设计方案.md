...
>
> ⚠️ **已过时机制提示（2026 更新）**：本文中关于「英灵殿 Valhalla / 死亡飞升 / 传承重生 / `task.inherit` / `ValhallaEntry` / generation+1 换代」的部分已不再适用。
> 归档传代机制已整体移除，替换为**自动对话压缩**：当数字成员会话 token 达到阈值时，系统自动把较早的对话历史压缩成摘要并继续同一段会话（不换代、不退场、无任何 Valhalla 记录），实现见 `server/api/services/conversation_compress.py`。
> 凡涉及英灵殿/死亡/传承演出与 `valhalla` 数据契约的内容仅作历史留档，落地时请以自动压缩模型为准。
...
> 当前固定建筑为传承知识库、英灵殿、出生地三座。议事厅已移除，成员单击直接打开隔空对话。
...
| 角色/状态 | 锚区行为 |
| --- | --- |
| 核心管理员（数字社会管理员） | 锚定图书馆前的小广场，在该范围踱步 |
...
任务开始 → 目标成员头顶显示“任务卷轴”气泡；有端侧绑定时作坊动效启动。任务完成（`task_recent_completed` 更新 / task:result）→ 头顶显示 ✅，作坊烟囱熄火。
...
| 期 | 内容 | 验收标准 |
| --- | --- | --- |
| **P0 观察者**（MVP） | game 入口 + tilemap 草原 + 3 固定建筑 + 作坊随 `agent:list` 增减 + 成员按 §4.3 锚区站位/游荡 + hover tooltip + token 预警气泡。只读。 | 打开 `/game/` 能实时反映当前数字社会全貌；dashboard 加 iframe 入口 |
...
| **P3 进化与实战**（已裁剪范围，2026-06-11） | ① **信使**（已完成）：`message.send_to_ai` 可视化为信封从发信成员飞向收信成员 ② **项目分区**：每个 project 一片围栏领地（按需立项）③ ~~进化竞技场实体化~~、~~多人观战~~、~~时间轴回放~~（需求方确认不做）④ 桌面 agent 侧小窗（按需） | 信使已上线；其余按需立项 |

---

## 9. 风险与开放问题

1. **固定建筑**：采用传承知识库、英灵殿、出生地三座；议事厅于 2026-06-12 移除。
