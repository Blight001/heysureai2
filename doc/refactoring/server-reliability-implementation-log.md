...
- pytest：282 个 unit/contract 测试通过，4 个 PostgreSQL integration 用例由 CI 执行
...
- `core.py` 已提取通信 Prompt、调试支持、推理预算策略、工具名解析、跨 Runtime 客户端、历史消息构建器、计划自动收尾服务、工具执行器和截图媒体策略；有效行数 2933 → 2049，`_run_worker_impl` 1626 → 1378。
...
- 工具批次控制流使用 `TurnCallAction` 显式枚举；未执行调用闭合、截图延迟刷新和重复拒绝计数已移出 `_execute_turn_call`。普通调用与拼接兼容调用共用不可变 `ToolExecutionResult`，统一同步桥接、端侧长超时、失败归一化、展示文本和耗时，函数 539 → 501 行；相关状态、路由与执行契约新增 10 个测试。
...
- 根仓库已自动更新到 `89fc5e5`，Server 子模块对应 `698a6a7`；管理员版本端点与四 Runtime 状态均已核验。
