...
- pytest：263 个 unit/contract 测试通过，4 个 PostgreSQL integration 用例由 CI 执行
...
- `admin.py` 已提取 Runtime 服务客户端，并将服务/ChatRun 的 5 条路由及用户管理的 5 条路由分别迁入 `admin_runtime_routes.py`、`admin_user_routes.py`；有效行数 1349 → 982、直接依赖 23 → 19，新增 7 个客户端与路由契约测试。

## 2026-08-09 部署环境验收

- 根仓库已自动更新到 `bf29edf`，Server 子模块对应 `db6f953`；管理员版本端点与四 Runtime 状态均已核验。
