...
- pytest：290 个 unit/contract 测试通过，4 个 PostgreSQL integration 用例由 CI 执行
...
- `core.py` 已提取通信 Prompt、调试支持、推理预算策略、工具名解析、跨 Runtime 客户端、历史消息构建器、计划自动收尾服务、工具执行器、截图媒体策略和工具结果持久化服务；有效行数 2933 → 1822，`_run_worker_impl` 1626 → 1348。
...
- 工具批次控制流使用 `TurnCallAction` 显式枚举；未执行调用闭合、截图延迟刷新和重复拒绝计数已移出 `_execute_turn_call`。普通调用与拼接兼容调用共用不可变 `ToolExecutionResult`；拼接兼容批次改为 `JoinedToolEvent` 迭代器，停止、跳过与完成结果均为显式事件并保持逐项持久化顺序；todo 旧参数推断、describe 结果解析和控制工具回传也已提取。函数 539 → 471 行、复杂度 98 → 78，相关状态、路由与执行契约新增 15 个测试。
...
- 根仓库已自动更新到 `4b1244a`，Server 子模块对应 `c5ab698`；管理员版本端点与四 Runtime 状态均已核验。
