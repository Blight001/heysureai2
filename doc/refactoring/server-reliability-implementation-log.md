...
- Python 复杂度债务：从 284 项降至 262 项，并已收紧机器基线
- 架构依赖债务：从 47 项降至 43 项，并已收紧机器基线
- pytest：287 个 unit/contract 测试通过，4 个 PostgreSQL integration 用例由 CI 执行
...
- `core.py` 已提取通信 Prompt、调试支持、推理预算策略、工具名解析、跨 Runtime 客户端、历史消息构建器、计划自动收尾服务、工具执行器和截图媒体策略；有效行数 2933 → 2023，`_run_worker_impl` 1626 → 1348。
...
- 根仓库已自动更新到 `691e7f8`，Server 子模块对应 `60faf68`；管理员版本端点与四 Runtime 状态均已核验。
