...
- [x] 建立整改分支和问题清单（`codex/server-reliability-completion`，问题与证据持续记录在实施日志）。
- [x] 记录复杂度、测试、覆盖率和运行指标基线。
- [x] 保存本次故障时间线、日志证据和根因。
...
- [x] 引入模拟 Agent 合同测试。
- [x] 完成超时、取消、重连、迟到回执测试矩阵（取消会事务性终结 dispatch、释放 waiter/设备队列并拒绝迟到回执复活）。

验收：所有设备调度状态转换均有单测和至少一条集成测试。

### 阶段 3：AI 推理核心拆分（2–4 周）

- [x] 冻结 `_run_worker_impl` 新功能入口；其现为 8 行稳定适配入口，外层状态流由 `WorkerRunMachine` 承担。
...
- [x] 每次拆分保持行为等价并执行回归测试（当前 387 个 unit/contract 测试通过）。
...
- [x] 拆分 `admin.py` 和首批超长路由（`admin.py` 已降至 493 行，其它清单继续按基线治理）。
- [x] 清理 `main/api/` 导入副作用（首批 package/app factory 与 Agent Socket 所有权已清理）。
- [x] 建立架构依赖检查。
- [x] 将兼容逻辑移入 Alembic 或明确适配层（遗留数据库接管仅由显式 `_legacy_adopt` 适配边界调用）。
...
- [x] 执行登录、AI、MCP、模拟设备 E2E（部署环境已完成真实模型 + `mcp.describe`/工具调用及模拟 Agent 往返验证）。
- [x] 注入进程终止、网络超时、数据库锁和设备断线（重启演练、故障矩阵和真实 PostgreSQL advisory-lock 集成测试）。
- [x] 建立发布前自动冒烟与发布后观测窗口（readiness-gated smoke、管理员版本/服务状态核验和失败日志采集）。
- [x] 建立自动回滚脚本。
...
- [x] 每个整改迭代至少消除或收紧一个超限文件/函数（当前 284 → 243）。
- [x] 基线只能下降，不能随意上调（CI 与脚本均拒绝新增条目或数值增长）。
- [x] 每月复查慢请求、锁等待和任务超时 Top N（已提供脱敏查询脚本和月度运维清单）。
- [x] 删除过期兼容层、豁免和临时补丁（移除无调用的 core 兼容别名与 MCP loader 空操作，保留项均有明确边界和退出条件）。
...
- [x] 生产文件无新增超过 500 行。
- [x] 业务函数无新增超过 80 行或复杂度超过 15。
- [x] 关键超长核心已拆分并具有清晰职责。
- [x] Runtime 启动不执行隐式 DDL。
- [x] 所有任务状态机具备合法转换与终态保证。
- [x] 四进程具备标准 readiness 和 draining。
- [x] 滚动发布可自动验证并在失败时停止/回滚。
- [x] PostgreSQL 锁、长事务、队列年龄可观测。
- [x] 真实 PostgreSQL 集成测试进入 CI。
- [x] 模拟设备 E2E 覆盖成功、失败、超时、断线和重连。
- [x] 关键模块增量覆盖率不低于 85%，状态机分支覆盖率不低于 90%（本批关键模块合并分支覆盖率 97.43%，`WorkerRunMachine` 100%）。
- [x] 连续故障演练不产生永久 `running`/`pending` 数据。
- [x] 运维文档与实际部署脚本保持一致。
