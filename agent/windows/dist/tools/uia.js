...
function toPhysicalRect(win, rect) {
    const content = win.getContentBounds();
    const display = electron_1.screen.getDisplayMatching(content);
...
async function inspectOwnElectronDom(args = {}) {
...
`, true);
    const indexed = (Array.isArray(elements) ? elements : []).map((element, index) => ({
        ...element,
        index,
...
async function clickOwnElectronDom(args = {}) {
    const win = findOwnElectronWindow(args.title);
    if (!win)
        return null;
...
function psNullableString(value) {
    if (value === undefined || value === null || String(value).length === 0)
        return '$null';
    return psString(String(value));
}
function psInt(value, fallback) {
...
`;
    const { stdout, stderr, exitCode } = await (0, powershell_1.runPowerShellScript)(script);
    if (!stdout) {
...
async function uiClick(args = {}) {
    const hasTarget = (args.name !== undefined && args.name !== null && String(args.name).length > 0) ||
