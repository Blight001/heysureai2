...
let overrides = {};
let toolEnabled = {};
const PARENT_ORDER = ['sensory', 'learning', 'tool', 'other'];
const PARENT_META = {
...
function toolGroupKey(name) {
    ...
    return KNOWN_GROUPS.has(key) ? key : 'other';
}
function groupMeta(name) {
    const key = toolGroupKey(name);
    return GROUP_META[key] || { zh: toTitleCase(key), en: toTitleCase(key).toUpperCase(), parent: 'other' };
}
...
function renderGroup(parent, group, tools) {
    if (!tools.length)
        return '';
...
function renderParentSection(parent, childMap, open = false) {
    const orderedGroups = parent === 'other'
        ? Array.from(childMap.keys()).sort()
...
function toolDataValue(tools) {
    return tools.map(t => t.name).join('|');
}
function toolsFromData(value) {
...
async function setToolEnabled(name, enabled) {
    if (!name)
        return;
    if (enabled)
        delete toolEnabled[name];
    else
        toolEnabled[name] = false;
    renderMcpList();
    await window.heysureAPI.mcpSetEnabled({ tool: name, enabled });
    await loadMcp();
}
async function setToolsEnabled(names, enabled) {
...
async function loadMcp() {
    const list = $('mcp-list');
    const openParents = new Set(Array.from(list.querySelectorAll('details.mcp-parent[open]')).map(el => el.dataset.parent || ''));
...
function renderMcpList(openParents = new Set(), scrollTop = 0) {
    const list = $('mcp-list');
    const parentMaps = new Map();
    for (const parent of PARENT_ORDER)
        parentMaps.set(parent, new Map());
    for (const tool of toolDefs) {
        const meta = toolMeta(tool.name);
        const parent = meta.parent;
        if (!parentMaps.has(parent))
            parentMaps.set(parent, new Map());
    ...
    list.querySelectorAll('[data-toggle-tools]').forEach(el => {
        el.addEventListener('click', e => e.stopPropagation());
