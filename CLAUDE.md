...
| `server/` | Python + FastAPI + Socket.IO | 后端，拆成 4 个进程 | [`server/CLAUDE.md`](server/CLAUDE.md) |
| `device/` | Electron / Chrome MV3 / Kotlin + TS | 端侧执行器：桌面 + 浏览器扩展 + Android | [`device/CLAUDE.md`](device/CLAUDE.md) |
| `doc/` | Markdown | 角色 prompt 与设计文档 | `doc/prompt/` |
...
# 本地分进程（Windows）
server\run.bat          # 平铺打开 4 个后端窗口
web\run.bat             # 前端 dev server (58150)
device\windows\run.bat  # Windows 桌面端壳

# 手动单进程（需 PYTHONPATH=main:.）
cd server
...
# 健康检查
curl http://127.0.0.1:3000/   # → {"message":"HeySure Server is running"}
curl http://127.0.0.1:3001/   # mcp runtime
curl http://127.0.0.1:3003/   # ai runtime
```
