...
> 同内容的 Agent 版见 [`AGENTS.md`](AGENTS.md)（Codex / 通用 agent 读取）。
...
| 仓库            | 本地目录 | 技术栈 | 作用 | 详细文档 |
|-----------------|----------|--------|------|----------|
| HeySure-Web     | `deploy/web/`   | Vue 3 + Vite + TS | 前端控制台 | [`deploy/web/CLAUDE.md`](deploy/web/CLAUDE.md) / [`deploy/web/AGENTS.md`](deploy/web/AGENTS.md) |
| HeySure-Server  | `deploy/server/`| Python + FastAPI + Socket.IO | 后端（4 进程） | [`deploy/server/CLAUDE.md`](deploy/server/CLAUDE.md) / [`deploy/server/AGENTS.md`](deploy/server/AGENTS.md) |
| HeySure-Device  | `device/`| Tauri / Chrome MV3 / Kotlin / Python | 端侧执行器 | [`device/CLAUDE.md`](device/CLAUDE.md) / [`device/AGENTS.md`](device/AGENTS.md) |
