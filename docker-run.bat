@echo off
cd /d "%~dp0"
rem Server deployment uses only deploy/server and deploy/web submodules.
rem After clone: git submodule update --init --recursive -- deploy/server deploy/web
...
if not exist deploy\server\other\scripts\repo-updater.py (
  ...
  if errorlevel 1 (
    echo [ERROR] Failed to initialize deploy/server and deploy/web.
    exit /b 1
  )
)

if not exist deploy\server\logs mkdir deploy\server\logs
...
if errorlevel 1 (
  ...
  start "HeySure Repo Updater" /min cmd /c "set HEYSURE_REPO_ROOT=%CD%&& set HEYSURE_REPO_UPDATER_HOST=0.0.0.0&& set HEYSURE_REPO_UPDATER_PORT=%HEYSURE_REPO_UPDATER_PORT%&& set HEYSURE_REPO_UPDATER_TOKEN=%HEYSURE_REPO_UPDATER_TOKEN%&& python deploy\server\other\scripts\repo-updater.py > deploy\server\logs\repo-updater.log 2>&1"
  timeout /t 2 /nobreak >nul
  curl -fsS -H "Authorization: Bearer %HEYSURE_REPO_UPDATER_TOKEN%" "http://127.0.0.1:%HEYSURE_REPO_UPDATER_PORT%/version" >nul 2>nul
  if errorlevel 1 (
    echo [WARN] Repo updater failed to start or token mismatch. See deploy\server\logs\repo-updater.log
  ) else (
