...
const revision = (value: any) => crypto.createHash('sha256').update(JSON.stringify(value)).digest('hex')
...
export function reloadDynamicMcp(): { tools: number; local: number; server: number; revision: string; file: string } {
  ...
  replaceDynamicTools(next.map(def => ({
    ...
    implementation: {
      kind: 'dynamic', definition: def, code_kind: def.code_kind || 'program', code: def.code,
...
function scheduleReload(): void {
  ...
  reloadTimer = setTimeout(() => {
    try { reloadDynamicMcp(); changeListener?.() } catch (err) { console.error('Dynamic MCP reload failed:', err) }
  }, 120)
}
...
type ResolvedSource = { requested_path: string; path: string; root: string; absolute_path: string; fallback: boolean }
...
function readAppSource(requested: string): Record<string, any> {
  ...
  if (!relative || path.isAbsolute(relative) || relative.split('/').includes('..')) throw new Error('source_path must be a relative path inside the device application')
...
function inspectTool(name: string, includeSource = true): Record<string, any> {
  ...
  return {
    ok: true, name, implementation_kind: dynamic ? 'dynamic_override' : 'builtin',
    active_definition: dynamic || { name, description: active?.description || builtin?.description || '', input_schema: active?.inputSchema || builtin?.inputSchema || {} },
    active_handler_source: String(active?.handler || ''), builtin_handler_source: builtin ? String(builtin.handler) : null,
    source_files: sourceFiles, available_source_files: availableSources.map(item => item!.path), source_resolution: availableSources,
    source_roots: sourceRoots(), sources: includeSource ? readToolSources(name) : undefined,
    ...
    edit_workflow: [
      ...
      `Call ${MANAGER_TOOL} action=delete to restore the built-in implementation.`,
    ],
    starter_definition: dynamic || {
      name, description: active?.description || builtin?.description || `Dynamic override for ${name}`,
      input_schema: active?.inputSchema || builtin?.inputSchema || { type: 'object', properties: {} },
      code: [{ op: 'call', tool: `builtin:${name}`, args: '${args}', save_as: 'original_result' }, { op: 'return', value: '${vars.original_result}' }],
    },
  }
}
...
export const DYNAMIC_MCP_MANAGER_DEFINITION: ToolDefinition = {
  id: MANAGER_TOOL, platform: 'all', destructive: true,
  ...
  implementation: { kind: 'builtin_manager', source_files: ['src/executor/dynamic.ts', 'dist/executor/dynamic.js'], editable_via: MANAGER_TOOL },
