...
- [x] 总督（玩家化身，2026-06-13）：新增 `actors/GovernorActor.ts`——无 token / 无任务，
      故**不显示血条**；左下角"🎮 操控总督"开关（或按 G）进入跟随视角，相机锁定总督，
      **WSAD** 移动、走到 AI 成员附近（≤96px）头顶提示"按 F 交互"、按 **F** 在底部面板
      打开该成员信息；纯前端化身（金色光环 + 铭牌区别于核心管理员），不触达后端
