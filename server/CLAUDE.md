...
| 知识/进化工坊 | `workshop/`（**服务端内置**，每账号自动上线；`direction.md`/`policy.py`/`tools.py` 即控制面，`engine.py` 执行编排），绑定接口 `gateway/routers/workshop.py` + `api/workshop_bindings.py`（**1:1**，只能绑定一个数字成员）；工具实现在 `mcp_runtime/mcp/tools/{librarian,evolution}.py` 但不在内置 registry，AI 须绑定工坊才可调用 |
