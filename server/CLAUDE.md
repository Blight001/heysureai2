...
| 知识/进化工坊接入 | `gateway/routers/workshop.py`（执行回调 + 绑定），`api/workshop_bindings.py`；工具实现仍在 `mcp_runtime/mcp/tools/{librarian,evolution}.py`，但已不在内置 registry，须 AI 绑定 `agent/workshop/` 才可调用 |
