...
DEFAULT_MCP_DYNAMIC_RULE = "系统提示的[可用MCP工具]目录会一次性列出全部可调用工具的名称与简介，模型据此直接定位，无需先用 mcp.list_tools 浏览。需要参数时用 mcp.describe_tool（支持 tool 单个、tools 批量或 query 关键词搜索）取 schema；被加载的目标工具会在随后轮次直接可调用。"
