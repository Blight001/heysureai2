...
SYSTEM_PROMPT_FIELDS = {
    ...
    "prompt_ai_message_inbound": "AI 间消息中断提示 prompt（旧版兜底）",
    "prompt_ai_message_notify": "AI 间消息·通知模板（notify / 旧版 require_reply=false）",
...
SYSTEM_PROMPT_USAGE = {
    ...
    "prompt_ai_message_inbound": "兼容旧版：当 ai.send_message 没有显式指定 message_type 且 require_reply=true 时仍会注入这个。建议在新流程里直接配置 inquiry/reply/chitchat。",
