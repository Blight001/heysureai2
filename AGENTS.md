...
| 仓库            | 本地目录 | 技术栈 | 作用 | 详细文档 |
|-----------------|----------|--------|------|----------|
| HeySure-Web     | `web/`   | Vue 3 + Vite + TS | 前端控制台 | [`web/AGENTS.md`](web/AGENTS.md) |
| HeySure-Server  | `server/`| Python + FastAPI + Socket.IO | 后端（4 进程） | [`server/AGENTS.md`](server/AGENTS.md) |
| HeySure-Device  | `device/`| Electron / Chrome MV3 / Kotlin + TS | 端侧执行器 | [`device/AGENTS.md`](device/AGENTS.md) |
